<template>
  ...
  <button
    class="saveworkAnnouncementInfo"
    @click="sendWorkAnnouncement(textareaWork,[],1)"
  >发布</button>
  <button
    class="deleteworkAnnouncementInfo"
    @click="clearWorkAnnouncement"
  >清空</button>
  ...
  <button
    class="saveDirectionInfo"
    @click="sendDirectionAnnouncement(textareaDirection,[],directionKey+2)"
  >发布</button>
  <button
    class="deleteDirectionInfo"
    @click="clearDirectionAnnouncement"
  >清空</button>
  ...
  >
    <div
      v-for="(input, index) in danmakuInputs"
      :key="index"
      class="smallDanmakuContainer"
    >
      <el-input
        class="danmaku-input"
      ...
      />
      <button
        class="saveDanmakuInfo"
        v-if="isShowAddContainer"
        @click="sendDanma(input.content,[],8)"
      >发布</button>
      <button
        class="deleteDanmakuInfo"
        v-if="isShowAddContainer"
        @click="clearDanma(index)"
      >清空</button>
    </div>
  </div>
